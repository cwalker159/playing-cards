...
public class Trick {
  ...
  public static void main(String[] args) {
    //DONE Create instance of a Deck and shuffle it.
  ...
  private static void tallyPiles(List<Card> blackPile, List<Card> redPile) {
    ...
    redPile.sort(
        Comparator
            .comparing(Card::getSuit, Comparator.comparing(Suit::getColor).reversed())        //.reversed() will reverse the comparison my color
            .thenComparing(Comparator.naturalOrder())
    );
  ...
  private static void splitDeck(Deck deck, List<Card> blackPile, List<Card> redPile) {        //Refactored method from a "while" loop.
    while(!deck.isEmpty()) {
